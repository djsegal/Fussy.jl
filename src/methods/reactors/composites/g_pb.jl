@symbol_func function G_PB(cur_reactor::AbstractReactor)
  ...
  cur_denominator = ( 1 - K_CD(cur_reactor) * cur_reactor.sigma_v ) / cur_reactor.T_bar

  cur_denominator -= K_BS(cur_reactor) / safe_symbol(cur_reactor, :I_P)

  cur_denominator /= sqrt(cur_reactor.T_bar)
