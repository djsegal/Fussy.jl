...
function calc_possible_values(cur_value=symbol_dict["sigma_v_hat"], cur_symbol=symbol_dict["sigma_v_hat"])
  ...
  cur_K_CD_denom_symbol = Tokamak.symbol_dict["K_CD_denom"]
  ...
  if cur_T_k_type != SymPy.Sym
    ...
    still_has_symbols = has(cur_value, Tokamak.symbol_dict["R_0"])
    still_has_symbols |= has(cur_value, Tokamak.symbol_dict["B_0"])
