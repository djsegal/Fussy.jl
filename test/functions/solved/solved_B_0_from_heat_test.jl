...
@testset "Solved B 0 From Heat Function Tests" begin

  @test isdefined(Tokamak, :solved_B_0_from_heat) == true
  ...
  for cur_bool in [false, true]
    ...
    for cur_T_k in linspace(10, 15, 10)

      Tokamak.load_input( "T_k = $(cur_T_k)u\"keV\"" )

      actual_value = Tokamak.solved_B_0_from_heat()
      actual_value /= 1u"T"
