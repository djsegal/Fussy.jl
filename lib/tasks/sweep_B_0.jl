...
function sweep_B_0(B_list; verbose=true)
  ...
  solved_equations["B_0"] = Array{Float64}(B_list)

  solved_equations["success"] = Array{Bool}(length(B_list))

  fill!(solved_equations["success"], false)

  solved_equations["primary_constraint"] = Array{AbstractString}(length(B_list))

  fill!(solved_equations["primary_constraint"], "x")

  solved_equations["secondary_constraint"] = Array{AbstractString}(length(B_list))
  ...
  if enable_eta_CD_derive && any(x -> !isnan(x), solved_equations["eta_CD"])
    _resweep_side_B_0_s(solved_equations, B_list, verbose)
  end
...
function _sweep_B_0(solved_equations, B_list, cur_range, cur_primary_constraint, cur_secondary_constraint, verbose; is_left_branch=false, has_bad_parent=false, is_initial_run=false)
  ...
  elseif length(success_indices) == 1
    ...
    side_guess = Interpolations.interpolate((cur_B_0_grid,), cur_T_k_grid, Gridded(Linear()))[main_value]
    cur_eta_CD = Interpolations.interpolate((cur_B_0_grid,), cur_eta_CD_grid, Gridded(Linear()))[main_value]
  end
  ...
  if !is_successful_run
    if length(beg_range) == 0
      ...
      good_beg_value = _sweep_B_0(
        solved_equations, B_list, first(beg_range),
  ...
  if !is_successful_run
    if length(end_range) == 0
      ...
      good_end_value = _sweep_B_0(
        solved_equations, B_list, last(end_range),
        cur_primary_constraint, cur_secondary_constraint, verbose,
  ...
  _sweep_B_0(solved_equations, B_list, beg_range, cur_primary_constraint, cur_secondary_constraint, verbose, is_left_branch=left_is_left, has_bad_parent=!is_successful_run)
  _sweep_B_0(solved_equations, B_list, end_range, cur_primary_constraint, cur_secondary_constraint, verbose, is_left_branch=right_is_left, has_bad_parent=!is_successful_run)

  return is_successful_run

end

function _resweep_side_B_0_s(solved_equations, B_list, verbose)
  ...
  for cur_entry in values(difference_dict)
    ...
    for cur_index in cur_entry["cur_indices"]
      ...
      _sweep_B_0(solved_equations, B_list, cur_index, solved_equations["primary_constraint"][cur_neighbor], solved_equations["secondary_constraint"][cur_neighbor], verbose)
